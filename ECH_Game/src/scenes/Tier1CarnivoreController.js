...
export default class Tier1CarnivoreController extends ScriptNode {
	...
	setupStateMachine() {
		...
		const chaseSpeed = 180;         // chase movement speed (set per carnivore/aggressive herb)
		const eatMoveSpeed = 80;        // speed while moving toward food/corpse

		// Loiter behaviour
		const loiterRadius = 400;               // how far from home position creature will wander (px)
		...
		// Combat
		const powerValue = 5;           // power value for combat resolution — higher wins
