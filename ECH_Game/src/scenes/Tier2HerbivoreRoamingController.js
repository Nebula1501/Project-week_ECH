...
export default class Tier2HerbivoreRoamingController extends ScriptNode {
	...
	setupStateMachine() {
		...
		const detectionRadius = 175;

		// Movement speeds (px per second)
		const neutralSpeed = 50;        // loiter/patrol movement speed
		const fleeSpeed = 200;          // flee movement speed
		const chaseSpeed = 120;         // chase movement speed (set per carnivore/aggressive herb)
		const eatMoveSpeed = 75;        // speed while moving toward food/corpse

		// Loiter behaviour
		const loiterRadius = 125;               // how far from home position creature will wander (px)
		const directionChangeInterval = 3000;   // how often creature picks a new loiter direction (ms)
		const loiterPauseDuration = 2000;       // pause duration between direction changes (ms)
		const returnPauseDuration = 2000;       // pause before returning to home position (ms)

		// Flee behaviour
		const fleeDuration = 3000;      // how long creature flees before re-evaluating (ms)

		// Eating behaviour
		const eatDuration = 5000;       // how long eating animation lasts before food/corpse is destroyed (ms)
