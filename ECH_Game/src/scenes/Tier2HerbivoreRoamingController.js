...
export default class Tier2HerbivoreRoamingController extends ScriptNode {
	...
	setupStateMachine() {
		...
		const tuning = this.scene.creatureTuning?.tier2HerbivoreRoaming ?? {};
		...
		if (!stateDecider || !stateManager) {
			console.warn('Tier2HerbivoreRoamingController: missing StateDecider or StateManager');
			return;
		}

		// Apply detection radius
		if (go._detectionRadius) go._detectionRadius.radius = tuning.detectionRadius ?? 175;

		// Apply neutral/patrol behaviour values
		if (neutral) {
			neutral.moveSpeed = tuning.neutralSpeed ?? 50;
			neutral.loiterRadius = tuning.loiterRadius ?? 125;
			neutral.directionChangeInterval = tuning.directionChangeInterval ?? 3000;
			neutral.pauseDuration = tuning.loiterPauseDuration ?? 2000;
			neutral.returnPauseDuration = tuning.returnPauseDuration ?? 2000;
		}

		// Apply flee behaviour values
		if (flee) {
			flee.moveSpeed = tuning.fleeSpeed ?? 200;
			flee.fleeDuration = tuning.fleeDuration ?? 3000;
		}

		// Apply opportunity/eat behaviour values
		if (opportunity) {
			opportunity.moveSpeed = tuning.eatMoveSpeed ?? 75;
			opportunity.eatDuration = tuning.eatDuration ?? 5000;
		}
